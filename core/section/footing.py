...
.. important::
   ...
   断面照査が断面力を受け取るのと同じ設計)。断面力そのもの(杭反力・
   底版自重・上載土重量・浮力から各照査位置の M・S を求める計算)は
   **未実装**である。原典の計算例は底版を格点モデル(柱中心・杭中心に
   集中荷重を載荷する骨組)として解いており、柱の位置・寸法という本ソフトが
   持っていない諸元を必要とする。単純な片持ち梁として近似することもできるが、
   計算例が多柱・多列配置でそれに当たらず**検証できない**ため、推測で
   実装することは避けた(docs/VERIFICATION.md 第58回)。
